...
# Usage: nohup bash extract_mean_mpf_old_mpf_wmparc.sh > output.log 2>&1 &

# This script computes mean MPF values for each region in wmparc.mgz from 1) the fs parcellation of the MPF (volumes
# generated WITHOUT registering component images), while excluding all MPF voxels with values below 200 

set -e #stop on errors

FS_DIR_MPF="freesurfer_output" 			  # directory with MPF where component images are not registered prior to MPF volume reconstruction
OUTPUT_DIR="avg_MPF_nonreg_values"      	  # directory to write output mean MPF stats file to
TEMP_DIR="./temp_wmparc_files_nonregMPF" 	  # directory to write intermediate files to 
CTAB="$FREESURFER_HOME/FreeSurferColorLUT.txt"    # file with region names used by mrisegstats

mean_mpf_mpfparc="$OUTPUT_DIR/allsubjects_mpf_mean_oldmpfparc.csv"   #  name and path of output mean MPF file for MPF parcellation
sd_mpf_mpfparc="$OUTPUT_DIR/allsubjects_mpf_stdev_oldmpfparc.csv"   #  name and path of output std MPF file for MPF parcellation
...
for mpf_top_dir in "$FS_DIR_MPF"/H??-?_MPFcor_freesurfer; do    # for every directory with MPF reg data
	...
	subj_id=$(basename "$mpf_top_dir" | sed 's/_MPFcor_freesurfer//')  # extract subject ID
	...
	# Use old nonregistered MPF parcellation
